...
const DisplayProfile = ({ profile }) => {
  const { name, avatar, email, venueManager, bookings } = profile;
      ...
      <div className="container mx-auto max-w-2xl mt-10">
        <div className="flex-col md:flex md:flex-row justify-between items-center max-w-sm mx-auto md:max-w-lg px-4 py-4 ">
          ...
          <div
            ...
            onClick={() => setActiveTab("expiredBookings")}
          >
            <h1 className="text-3xl font-customFont text-center">
              Your Expired Bookings
            </h1>
          </div>
        </div>
        {activeTab === "bookings" && (
        ...
        {activeTab === "venues" && (
          <div>{/* Add content for Your Venues tab */}</div>
        )}
        {activeTab === "expiredBookings" && (
          <div>{/* Add content for Your Expired Bookings tab */}</div>
        )}
      </div>
    </>
  );
